...
function sendMessage<TResponse>(message: unknown): Promise<TResponse> {
  return browser.runtime
    .sendMessage(message)
    .then((response?: TResponse | ErrorMessageResponse) => {
      if (!response) {
      ...
      if (
        ...
        throw new Error((response as ErrorMessageResponse).error);
      }
...
function readStoredSession(): Promise<unknown> {
  return browser.storage.local.get("authSession").then((items) => items.authSession);
}
