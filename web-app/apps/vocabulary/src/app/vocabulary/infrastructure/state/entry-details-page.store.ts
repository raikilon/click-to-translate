...
export class EntryDetailsPageStore {
  ...
  readonly usagesHasNext = computed(() => this.usagesPageIndex() + 1 < this.usagesTotalPages());
  ...
  async saveTerm(term: string): Promise<void> {
    await firstValueFrom(
      this.httpClient.patch<void>(`${this.vocabularyBasePath}/entries/${this.entryId()}`, { term }),
    );
  ...
  async deleteEntry(): Promise<void> {
    await firstValueFrom(
      this.httpClient.delete<void>(`${this.vocabularyBasePath}/entries/${this.entryId()}`),
    );
  }
