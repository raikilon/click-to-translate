...
} from "@/content/authentication/application/AuthErrors";
import { extensionIdentity } from "./identity";
import { authLogoutStorageItem } from "./AuthStorage";
...
export class AuthSessionManager
  implements AuthStateProvider, IAuthenticationService, IAccessTokenProvider
{
  ...
  private async loginByAuthorizationCode(
    ...
    const authorizeUrl = new URL(authRuntimeConfig.authAuthorizeUrl);
  ...
  private validateAuthConfig(): void {
    if (!authRuntimeConfig.authAuthorizeUrl.trim()) {
      throw new Error("Missing auth authorize URL configuration.");
    }

    if (!authRuntimeConfig.authTokenUrl.trim()) {
      throw new Error("Missing auth token URL configuration.");
    }

    if (!authRuntimeConfig.oauthClientId.trim()) {
  ...
  private createAuthorizationServer(
    ...
    return {
      issuer: issuerOverride ?? this.inferIssuer(authRuntimeConfig.authAuthorizeUrl),
      authorization_endpoint: authRuntimeConfig.authAuthorizeUrl,
      token_endpoint: authRuntimeConfig.authTokenUrl,
    };
  }

  private createClient(): OAuthClient {
    return {
      client_id: authRuntimeConfig.oauthClientId,
    };
  }
